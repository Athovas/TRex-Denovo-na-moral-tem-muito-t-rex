...
function setup() {
  ...
  chao=createSprite(width/2,height-50, 400,20)
  ...
  rex=createSprite(50,height-50,20,50)
...
function draw() {
  ...
  if((touches.length > 0 || keyDown("space")&&rex.y>= height-120))  {
    rex.velocityY=-10
    somsalto.play()
  }
  
   
    text("Pontuação:"+pontos, width/2,height/20)
...
function nuvens() {
  if(frameCount%75===0) {
  nuvem=createSprite(width/1,height-300, 40,10)
  nuvem.y=Math.round(random(10,400))
  ...
  function gerarobjetos() {
    if(frameCount%60===0) {
      var obstaculo=createSprite(width/1,height-55, 10,40)
